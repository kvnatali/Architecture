...
namespace database
{
    ...
    Users Users::read_by_id(long id)
    {
        ...
        Users Users::read_from_cache_by_login(std::string login)
    {
        ...
        try
        {
            ...
            if (database::Cache::get().get(login, result))
                ...
                throw std::logic_error("key (login) not found in the cache");
        }
    ...
    size_t Users::size_of_cache(){
        return database::Cache::get().size();
    }

    void Users::save_to_cache()
    {
        std::stringstream ss;
